...
const int LEFT_EYE_CENTER = 80;

const int RIGHT_EYE_MIN = 50;
const int RIGHT_EYE_MAX = 140;
const int RIGHT_EYE_CENTER = 95;
...
ServoAxis mouthAxis = {Servo(), MOUTH_PIN, MOUTH_CLOSED, MOUTH_CLOSED, 4, 0};
ServoAxis leftArmAxis = {Servo(), LEFT_ARM_PIN, ARM_CENTER, ARM_CENTER, 3, 0};
ServoAxis rightArmAxis = {Servo(), RIGHT_ARM_PIN, ARM_CENTER, ARM_CENTER, 3, 0};
ServoAxis leftEyeAxis = {Servo(), LEFT_EYE_PIN, LEFT_EYE_CENTER, LEFT_EYE_CENTER, 2, 0};
ServoAxis rightEyeAxis = {Servo(), RIGHT_EYE_PIN, RIGHT_EYE_CENTER, RIGHT_EYE_CENTER, 2, 0};
...
void attachAxis(ServoAxis& axis, const char* name)
{
  ...
  Serial.println(axis.servo.attached() ? "YES" : "NO");
}
...
void setImmediate(ServoAxis& axis, int target)
{
  axis.target = target;
  axis.current = target;
  axis.servo.write(axis.current);
...
void stepAxis(ServoAxis& axis, uint32_t nowMs)
{
...
{
  int left = triangleWave(nowMs, 1200, ARM_MIN, ARM_MAX);
...
void updateThinkingAnimation(uint32_t nowMs)
{
  ...
  int leftEye = triangleWave(nowMs, 700, LEFT_EYE_MAX, LEFT_EYE_MIN);
  int rightEye = triangleWave(nowMs, 700, RIGHT_EYE_MAX, RIGHT_EYE_MIN);
...
{
  int left = triangleWave(nowMs, 1000, ARM_MIN, ARM_MAX);
  int right = ARM_MIN + ARM_MAX - left;
  setTarget(leftArmAxis, left);
  setTarget(rightArmAxis, right);
  ...
  uint32_t elapsed = nowMs - speakingPlan.startedAtMs;
...
void updateStateMachine()
{
  ...
  stepAxis(mouthAxis, nowMs);
  stepAxis(leftArmAxis, nowMs);
  stepAxis(rightArmAxis, nowMs);
  stepAxis(leftEyeAxis, nowMs);
  stepAxis(rightEyeAxis, nowMs);
}
...
void mouthOpen()
{
  ...
  setImmediate(mouthAxis, MOUTH_OPEN);
}

void mouthHalf()
{
  Serial.println("MOUTH HALF");
  setImmediate(mouthAxis, MOUTH_HALF);
}

void mouthCloseSlow()
{
  Serial.println("MOUTH CLOSE");
  setImmediate(mouthAxis, MOUTH_CLOSED);
}
...
struct ServoAxis
{
  ...
  int current;
  int target;
  uint8_t stepDelayMs; // Ya no interfiere, pero lo dejamos para mantener la estructura original
  uint32_t lastStepMs;
};
...
// ==========================================
// FUNCIÓN CORREGIDA: Sin overhead de retraso. 
// Va directo al objetivo que se le pida.
// ==========================================
void stepAxis(ServoAxis& axis)
{
  if (axis.current == axis.target)
    return;
...
void updateStateMachine()
{
  ...
  // Actualizados para ya no depender del tiempo extra, se aplican directo
  stepAxis(mouthAxis);
